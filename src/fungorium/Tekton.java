...
public class Tekton {
    ...
    // Visszaadja a tektonon lévÅ‘ gombatestet.
    ...
    public List<GombaFonal> getGomaszFonalai(Gombasz g) { 
		...
		if (fonalak != null) {
            for (GombaFonal fonal : fonalak) {
				//Ha az adott fonál a megfelelÅ‘ gombászé, akkor hozzáadjuk a listához
    ...
    public boolean elhal(Gombasz g) {
		...
		bejarando.add(this);
		//Amíg a bejárandó lista nem üres és nem talalhato benne megfelelÅ‘ gombatest
		while (!bejarando.isEmpty()) {
			//Vesszük a következÅ‘ bejárandó tektont
			Tekton t = bejarando.get(0);
			//Menézzük, hogy van-e rajta gombatest
			if (!t.vanHely()) {
				//Ha van, akkor megnézzük, hogy a gombatest a megfelelÅ‘ gombászhoz tartozik-e
				if (t.getGombaTest().kie == g) return false;
			}
			//Ha nem, akkor megnézzük, hogy a tektonon lévÅ‘ gombászhoz tartozó fonalak tektonjai benne vannek-e már az egyik listában
		...
		//Ha a bejárandó listában nem találtunk megfelelÅ‘ gombatestet, akkor visszatérünk igaz értékkel
		return true;
	}
		...
		for (String part : parts) {
			...
			switch (attr) {
				...
				case "szomszedok":
					...
					//ezért elmentjük egy stringbe és késÅ‘bb beállítjuk
					szomszedokStr = value;
					break;
				case "sporakSzama":
				...
				case "gombaTest":
					...
					break;
				case "kapcsoltTekton":
					//Ugyan az a helyzet mint a szomszédokkal, ezért elmentjük egy stringbe és késÅ‘bb beállítjuk
					//a kapcsolt tektonokat
					kapcsoltTektonStr = value.replace("'", "");
					break;
				default:
					System.out.println("Ismeretlen attribÃºtum: " + attr);
					break;
			}
		}
